...
import { FaUtensils, FaCoffee, FaIceCream, FaPizzaSlice, FaHamburger } from "react-icons/fa"; // Icons for categories
        ...
        <div className="">
            <div className=" p-4 font-sans text-gray-800">
                ...
                <div className="flex overflow-x-auto gap-3 mb-6">
                    {/* Default "All" Option */}
                    <button
                        className={`flex flex-col items-center p-2 ${selectedCategory === "All" ? "text-[#ff8c00]" : "text-gray-600"
                            }`}
                        onClick={() => setSelectedCategory("All")}
                    >
                        <div className="bg-[#ff8c00] text-white rounded-full p-3">
                            <div className=" relative w-6 h-6">
                                <Image src="/foodicon.svg"
                    ...
                    {Object.keys(categories).map((categoryName) => (
                        <button
                            key={categoryName}
                            className={`flex flex-col items-center p-2 ${selectedCategory === categoryName
                                ? "text-[#ff8c00]"
                                : "text-gray-600"
                                }`}
                ...
                {/* Display Menu Items */}
                {Object.entries(filteredCategories).map(([categoryName, categoryItems]) => (
            ...
            <div className="text-center bg-[#ff8c00] text-white p-2 w-screen fixed bottom-0">
                <p className="text-sm">
                    Designed and custom-built for Aahaar by <Link href="https://ripple.relaef.com" className="underline font-medium">Ripple</Link> — a step towards the digital wave.
                </p>
            </div>
        </div>
    );
